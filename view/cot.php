<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        if(empty($_POST['cot'])){
            ...
            $msg[] = '<div class="error">COT must be of number type!</div>';
        }else{
            $enteredcot = mysqli_real_escape_string($con, trim($_POST['cot']));
        }
        if($enteredcot != $storedcot){
            $msg[] = '<div class="error">Incorrect COT</div>';
        }
        if(empty($msg)){
            ...
            // if(transferFunds($con, $current_bal, $id, $acct_no, $acct_name, $bank, $amount, $type, $status, $tr_id, $date) === TRUE){
            ...
            //     sendEmail($email, $subject, $mail);
                $successMsg = '<div class="success">COT Accepted</div> <p>Redirecting...</p>';
                header("refresh:5; url=./pin");
            // }else{
    ...
    <div class="card-container" style="position: relative;">
        <div class="card-details">
            <div class="withdraw">
                ...
                <div class="left"><p class="bold-history center">Identity verification</div>
            </div>
            <div class="withdraw">
            <div class="left"><p>By requesting the COT code, we ensure that only authorized users can access and perform transactions on your behalf, reducing the risk of fraud or identity theft.</div>
            </div>
        </div>
    </div>
    ...
    <div class="reg-container">
        <div class="pin">
            <div class="content">
                ...
                ?>
                <h4 class="align-center">COT REQUEST</h4>
                <p class="align-center">Enter your 4 digit transfer COT</p>
                <!-- <h3 class="align-center">Enter 6 digit OTP</h3> -->
                <form action="./cot" method="POST">
                    <span class="input-container"><input type="text" name="cot" id="password" maxlength="4" placeholder="Enter 4 digit COT"><i class="details-black fa fa-fingerprint"></i></span>
                    <button type="submit" class="submit-button white">Confirm COT</button>
                </form>
            </div>
        </div>
    </div>
